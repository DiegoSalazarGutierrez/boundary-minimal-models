...
For a composition $\lambda$ with exactly $m$ elements greater than $1$ and exactly $n$ elements equal to $1$, let $\lambda'$ be the partition obtained from $\lambda$ by removing all $1$s.
...
A \emph{Hamiltonian operator of $V$} is a diagonalizable operator $H \in \End(V)$ such that $H$ is a simultaneously a Hamiltonian operator of $V$ as a commutative associative algebra with unit $1$ (cf.\ appendix).
...
\begin{proposition}[{\cite[Proposition 2.3.1]{arakawa_remark_2012}} and my appendix]
  \label{prp:11}
  Let $R$ be a (graded) finitely generated Poisson algebra.
...
The (graded) vertex Poisson algebra structure on $JR$ given in \Cref{prp:11} for a (graded) finitely generated Poisson algebra $R$ will be called the \emph{level 0 vertex Poisson algebra structure of $JR$}.

If $R$ is a (graded) finitely generated Poisson algebra, and $M$ is an $R$-module, then we can verify that $JR \otimes_{R} M$ is a (graded) $JR$-module by defining the Poisson structure as
\begin{equation*}
  a_{(n)}(b\otimes u) = (a_{(n)}b)\otimes u + \delta_{n, 0}b\otimes\{a, u\} \quad \text{for $a \in R$, $b \in JR$, $u \in M$ and $n \in \mathbb{N}$}
\end{equation*}
and $H^{JR \otimes_{R} M}$ as in my papendix.
We have a natural (graded) inclusion $\inc: M \hookrightarrow JR \otimes_R M$, which is a (graded) $R$-module homomorphism.
The (graded) $JR$-module $M$ together with the (graded) $R$-module homomorphism $\inc: M \hookrightarrow JR \otimes_R M$ satisfy a universal property similar to that of $JR$ (see \Cref{sec:jet-algebras-jet}), as we now show.
...
\begin{example}
  \label{exa:10}
  We pick a highest weight $(c, h)$.
  From \Cref{exa:9}, $\gr_F(\Vir^c)$ is isomorphic to $\mathbb{C}[L_{-2}, L_{-3}, \dots]$.
  We have an isomorphism
  \begin{align*}
...
\begin{example}
  ...
  By \Cref{exa:9}, we have the following isomorphism of Poisson algebras
  \begin{align*}
  ...
  \end{equation*}
  From the equation $\ker(R_{\pi_c}) = \sigma_0(U(\Vir)\{a_{p, q}\})$ and \Cref{thr:23}, we obtain
  \begin{equation*}
...
We now assume that $R_V$ is (graded) finitely generated.
By the universal property of $JR_V$ (see \Cref{sec:jet-algebras-jet}), there is a (graded) differential algebra homomorphism $\phi_V: JR_V \to \gr_F(V)$ such that the following diagram commutes
\begin{equation*}
  \begin{tikzcd}
...
\begin{lemma}
  \label{lmm:26}
  Let $V$ be a (graded) vertex algebra such that $R_V$ is finitely generated.
...
\begin{example}
  \label{exa:12}
  Let $c \in \mathbb{C}$.
  Then $\Vir^c$ is classically free because $\gr_F(\Vir^c) \cong \mathbb{C}[L_{-2}, L_{-3}, \dots]$ by \Cref{exa:9} and $JR_{\Vir^c} = J(C[L_{-2}]) = \mathbb{C}[L_{-2}, L_{-3}, \dots]$ by \Cref{exa:16} and  \Cref{sec:jet-algebras-jet}.
...
\begin{example}
  ...
  \end{equation*}
  and $(b)_{\partial}$ is the differential ideal generated by $b$, cf.\ \Cref{sec:jet-algebras-jet} and \Cref{exa:9} where $\partial$ is denoted by $T$.
\end{example}
...
\begin{example}
  ...
  We consider $\mathbb{C}[L_{-2}, L_{-1}] = \bigoplus_{k \in \mathbb{N}}\mathbb{C}[L_{-2}]L_{-1}^k$ as a module over the Poisson algebra $\mathbb{C}[L_{-2}]$ with Poisson bracket given by $\{L_{-2}, L_{-1}^k\} = L_{-1}^{k + 1}$ for $k \in \mathbb{N}$.
  By \Cref{exa:10} and \Cref{exa:16}, we have the following isomorphism of modules over Poisson algebras
  \begin{align*}
  ...
  \begin{equation*}
    R_{L(c, h)} \cong \bigoplus_{k \in \mathbb{N}}\mathbb{C}[L_{-2}]L_{-1}^k/\sigma_0(J(c, h)).
  \end{equation*}
\end{example}
...
Let $V$ be a (graded) vertex algebra such that $R_V$ is finitely generated, and let $M$ be a (graded) $V$-module.
We have a natural (graded) inclusion $\inc: R_M \hookrightarrow \gr_F(M)$ of (graded) $R_V$-modules.
As we have explained in \Cref{sec:graded-vert-poiss}, we can consider $JR_V \otimes_{R_V} R_M$ as a (graded) $JR_V$-module.
...
\begin{proof}
  The assertion follows from \Cref{lmm:26} and \Cref{thr:55}.
\end{proof}
...
\begin{example}
  \label{exa:15}
  We pick a highest weight $(c, h)$.
  The Verma module $M(c, h)$ is always classically free as a $\Vir^c$-module because by \Cref{exa:10}, \Cref{exa:16} and \Cref{exa:13},
  \begin{align*}
...
We wish to determine when the irreducible modules $L(c_{p, q}, h_{m, n})$ over the simple Virasoro vertex algebras $\Vir_{p, q} = \Vir_{c_{p, q}}$ are classically free, see \Cref{thr:44} for details.
...
\begin{lemma}
  ...
  We have
  \begin{equation*}
    \ch_{L(c_{2, 2s + 1}, h_{1, i})}(q) = q^{h_{1, i}}p^{s, i}(1, q) = q^{h_{1, i}}\left(\prod_{\substack{n = 1 \\ n \not\equiv 0, \pm i \mod 2k + 1}}(1 - q^n)^{-1}\right).
  \end{equation*}
\end{lemma}
...
\begin{proof}
  ...
  \begin{align*}
    q^{h_{1, i}}p^{s, i}(1, q) &= q^{h_{1, i}}J_{s, i}(0, 1, q) \\
                               &= q^{h_{1, i}}\left(\prod_{\substack{n = 1 \\ n \not\equiv 0, \pm i \mod 2k + 1}}(1 - q^n)^{-1}\right) \\
...
We mostly consider $\mathbb{N}$-graded algebras, i.e., algebras $A$ such that $A_n = 0$ unless $n \in \mathbb{N}$.
...
Let $A$ be an $\mathbb{N}$-graded Poisson algebra with Hamiltonian $H$, and let $M$ be an $A$-module.
A Hamiltonian operator of $M$ is a diagonalizable operator $H^M \in \End(M)$ such that $H^M$ is a Hamiltonian of $M$ when $A$ is considered just an associative algebra and
\begin{equation*}
  H^M(\{a, u\}) = \{a, H^M(u)\} + \{H(a), u\} - \{a, u\} \quad \text{for $a \in A$ and $u \in M$}.
\end{equation*}

We mainly consider $h + \mathbb{N}$-graded $A$-modules for some $h \in \mathbb{C}$.
For an $h + \mathbb{N}$-graded $A$-module $M$ such that $\dim(M_{h + n}) < \infty$ for $n \in \mathbb{N}$, we define the \emph{character of $M$} by
\begin{equation*}
  \ch_M(q) = \sum_{n \in \mathbb{N}}\dim(M_{h + n})q^{h + n} \in q^h\mathbb{C}[[q]].
\end{equation*}

\section{Jet algebras and their modules}
...
Given a finitely generated algebra $R$, it is useful to consider the functor
\begin{equation*}
  JR \otimes_R \bullet: \text{$R$-Mod} \to \text{$JR$-Mod}
\end{equation*}
where $JR$ is merely considered an algebra.

Let $R$ be a graded finitely generated algebra  with Hamiltonian $H$.
We can extend uniquely the Hamiltonian $H$ to a Hamiltonian $H^{JR} \in \End(JR)$ because of \eqref{eq:1}.
