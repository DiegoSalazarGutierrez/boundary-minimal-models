...
\begin{example}
  ...
  \begin{align*}
    \pi^c: \Vir^c &\twoheadrightarrow \Vir_c, \\
    \pi^c(a) &= a + U(\Vir)\{a_{p, q}\}.
  \end{align*}
  Applying the functor $R$, we obtain an epimorphism
  \begin{equation*}
    R_{\pi^c}: R_{\Vir^c} \twoheadrightarrow R_{\Vir_c}.
  \end{equation*}
  From the equation $\ker(R_{\pi^c}) = \sigma_0(U(\Vir)\{a_{p, q}\})$ and \Cref{thr:23}, we obtain
  \begin{equation*}
    \ker(R_{\pi^c}) = \sigma_0(L_{-2}^{(p - 1)(q - 1)/2}\vac).
  \end{equation*}
  In summary,
  \begin{equation*}
