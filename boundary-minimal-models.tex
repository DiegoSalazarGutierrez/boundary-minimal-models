...
\begin{proof}
  ...
  \item $\lambda = [\underbrace{a + 1, \dots, a + 1}_{s - d}, \underbrace{a, \dots, a}_d]$ for some $a \in \mathbb{Z}_+$ and $d \in \{1, \dots, s\}$.
    ...
    \item[Subcase $a > 1$] In the sum \eqref{eq:6}, if $k > 0$, then $R^{l, k}_{n_1, \dots, n_{s - 1}}(L(z), \dots, L(z))|c_{2, 2s + 1}, h_{1, i}\rangle$ contributes $0$.
      We have
      \begin{equation*}
  ...
  From \eqref{eq:4} and \Cref{lmm:2}, we see that $\lp(u) = \lp((L_{-2}^s\vac)_{(s - sa + d - 1)}|c_{2, 2s + 1}, h_{1, i}\rangle) = L_{\lambda}|c_{2, 2s + 1}, h_{1, i}\rangle$.
...
\begin{proof}[Sketch of proof of \Cref{thr:2}]
  ...
  \end{align*}
  By Gröbner basis theory, \eqref{eq:1}, \Cref{rmk:4}, \Cref{lmm:10} and \Cref{lmm:11}, we have $\overline{B} \subseteq B$.
...
\begin{proof}[Proof of \Cref{thr:3}]
  ...
  From \eqref{eq:1}, \Cref{rmk:4}, \Cref{lmm:10} and the fact that $\lp$ is multiplicative, we obtain $u \in J(c_{2, 2s + 1}, h_{1, i})$ homogeneous such that $\lp(u) = L_{\eta_0}|c_{2, 2s + 1}, h_{1, i}\rangle$.
