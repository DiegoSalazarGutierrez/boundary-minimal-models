...
\begin{theorem}[{\cite[Theorem 2.15]{li_vertex_2004}}]
  ...
  \end{equation}
  Let $\lambda: \mathfrak{h} \to \mathbb{C}$ be a linear functional, where $\mathfrak{h}$ is a subalgebra of $\mathfrak{g}_+$ with $\mathfrak{h} \subseteq \ker(T_0)$.
  Given a smooth $(\mathfrak{g}, \mathfrak{F}, T_0)$-module $M$, where $h$ acts as $\lambda$, there is a unique module structure $Y^M: V^{\lambda}(\mathfrak{g}, \mathfrak{F}, T_0) \to \mathcal{F}(M)$ such that \eqref{eq:5} holds.
\end{theorem}
...
A \emph{Hamiltonian operator of $V$} is simultaneously a Hamiltonian operator of $V$ as a differential commutative associative algebra with derivation $T$ (see \Cref{sec:grad-poiss-algeb}) and a Hamiltonian operator of $V$ as a vertex Lie algebra.
...
By our definitions, $\partial$ descends to a derivation of $JR$, and we have an algebra monomorphism $\inc: R \hookrightarrow JR, \inc(x^j + (f_1, \dots, f_s)) = x^j_{(-1)} + (f_1, \dots, f_s)_{\partial}$ for $j = 1, \dots, r$.
