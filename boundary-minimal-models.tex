...
\begin{abstract}
  We determine when the irreducible modules $L(c_{p, q}, h_{m, n})$ over simple Virasoro vertex algebras $\Vir_{p, q}$, where $p, q \ge 2$ are relatively prime and $0 < m < p$ and $0 < n < q$, are classically free.
  When this happens, the modules can be expressed in terms of the jet algebra of the Zhu $C_2$-algebra.
  It turns out this only happens with the boundary minimal models, i.e., with the irreducible modules over $\Vir_{2, 2s + 1}$ for $s \in \mathbb{Z}_+$, and this is a consequence of Gordon's generalization of the Rogers-Ramanujan identities.
  As a corollary, we can express $\gr_F(L(c_{2, 2s + 1}, h_{1, i}))$ explicitly, for $s \in \mathbb{Z}_+$ and $i \in \{1, \dots, s\}$. \\
  \smallskip
...
In summary, two functors were constructed in \cite{li_abelianizing_2005}:
\begin{align*}
  \gr_F: \{\text{vertex algebras}\} &\to \{\text{vertex Poisson algebras}\}, \\
  \gr_F: \{\text{$V$-modules}\} &\to \{\text{$\gr_F(V)$-modules}\}.
\end{align*}

Given an algebra $R$, we can construct the jet algebra $JR$, which is the smallest differential algebra containing $R$.
It is proved by Arakawa in \cite[Proposition 2.5.1]{arakawa_remark_2012} that we have a natural surjection
\begin{equation*}
...
This problem is related to the question of whether one can recover the vertex algebra $V$ (or the $V$-module $M$) from $JR_V$ (or $JR_V \otimes_{R_V} R_M$), i.e., if it is possible to obtain quantization of $V$ because $\gr_F(V)$ and $V$ are often isomorphic in some way.

This problem was solved for the simple Virasoro vertex algebra $\Vir_{p, q}$ in \cite{van_ekeren_chiral_2021}, and the answer is very simple.
If $p > q \ge 2$ are relatively prime integers, then $\Vir_{p, q}$ is classically free if and only if $q = 2$.
The main objective of this article is to solve this problem for all irreducible modules over the simple Virasoro vertex algebra $\Vir_{p, q}$, namely $L(c_{p, q}, h_{m, n})$ for integers $m, n$ such that $0 < m < p$ and $0 < n < q$.

It turns out this problem is deeply connected with the Rogers-Ramanujan identities and Gordon's generalization \cite{gordon_combinatorial_1961}
\begin{equation*}
  \sum_{k = (k_1, \dots, k_{s - 1}) \in \mathbb{N}^{s - 1}}t^{kB^{(s)}_{s - 1}}\frac{q^{\frac{1}{2}kG^{(s)}k^T + kB^{(s)}_{s - i}}}{(q)_{k_1}\dots(q)_{k_{s - 1}}} = \prod^{\infty}_{\substack{n = 1 \\ n \not\equiv 0, \pm i \mod 2s + 1}}(1 - q^n)^{-1},
\end{equation*}
where $T$ denotes the transpose matrix, $s \in \mathbb{Z}_+$, $i \in \{1, \dots, s\}$, and
\begin{align*}
...
We can succinctly express Gordon's generalization of the Rogers-Ramanujan identities using the theory of partitions, as explained in \cite[\S8]{andrews_theory_1998}.
...
An infinite sequence $(f_n)_{n \in \mathbb{Z}_+}$ of nonnegative integers in which only finitely many of the $f_i$ are nonzero uniquely corresponds to a partition $[\underbrace{1, \dots, 1}_{f_1}, \underbrace{2, \dots, 2}_{f_2}, \dots]$.
Therefore, we have two ways of writing a partition.

Let $s \in \mathbb{Z}_+$, let $i \in \{1, \dots, s\}$, and we define the following sets of partitions:
\begin{align*}
...
\end{align*}
Gordon's generalization of the Rogers-Ramanujan identities becomes
\begin{equation*}
  A^{s, i} \sim P^{s, i}.
\end{equation*}
...
We now state our main theorems and show that the irreducible modules over the simple Virasoro vertex algebras $\Vir_{2, 2s + 1}$ are intimately related to the partition ideals $P^{s, i}$.
...
\begin{example}[$\ch_{M(c, h)}(t, q)$]
  \label{exa:5}
  We can consider the $\mathbb{Z}$-graded Virasoro Lie algebra $(\Vir, \mathbb{C}L_0 \oplus \mathbb{C}C)$.
...
\begin{proof}
  By the proof of \Cref{lmm:9}, \cite[Lemma 7.3]{andrews_theory_1998} and \cite[Corollary 2.9]{andrews_theory_1998}, we have
  \begin{align*}
...
\begin{proof}[Sketch of proof of \Cref{thr:2}]
  ...
  \end{align*}
  By Gröbner basis theory, \eqref{eq:2}, \Cref{rmk:5}, \Cref{lmm:11} and \Cref{lmm:12}, we have $\overline{B} \subseteq B$.
  Actually, \Cref{prp:1} and \Cref{lmm:10} imply $\overline{B} = B$.
...
\begin{proof}[Proof of \Cref{thr:3}]
  ...
  From \eqref{eq:2}, \Cref{rmk:5}, \Cref{lmm:11} and the fact that $\lp$ is multiplicative, we obtain $u \in J(c_{2, 2s + 1}, h_{1, i})$ homogeneous such that $\lp(u) = L_{\eta_0}|c_{2, 2s + 1}, h_{1, i}\rangle$.
...
As in the proof of \Cref{lmm:11}, let $w \in J(c_{2, 2s + 1}, h_{1, i})$ be the singular vector such that $\lp(w) = L_{-1}^i|c_{2, 2s + 1}, h_{1, i}\rangle$, and we set:
\begin{align*}
...
\begin{proof}
  ...
  Since $\{t_{\lambda} \mid \lambda \in P^{s, i}\}$ is a basis of $JR_{\Vir_{2, 2s + 1}} \otimes_{R_{\Vir_{2, 2s + 1}}} R_{L(c_{2, 2s + 1}, h_{1, i})}$, the set in \eqref{eq:10} is a basis of $R_{L(c_{2, 2s + 1}, h_{1, i})}$.
  ...
  Therefore, we can prove \eqref{eq:14} similarly to how we proved \eqref{eq:10}.
  The isomorphisms \eqref{eq:15} and \eqref{eq:16} follow.
\end{proof}
...
If $f: R_1 \to R_2$ is a homomorphism of finitely generated algebras, then $Jf: JR_1 \to JR_2$ is defined by requiring that $Jf(\partial_1^n(\inc_1(x))) = \partial^n_2(\inc_2(f(x)))$ for $x \in R_1$ and $n \in \mathbb{N}$ (c.f.\ \Cref{rmk:8}).
...
We can extend uniquely the Hamiltonian $H$ to a Hamiltonian $H^{JR} \in \End(JR)$ because of \eqref{eq:17}.
