...
The field of \emph{formal Laurent series}, denoted by $\mathbb{C}((x))$, is the subspace of elements $f(x) \in \mathbb{C}[[x^{\pm 1}]]$ such that there is $N \in \mathbb{Z}$ with $f_n = 0$ for $n \le N$.
...
We can consider $V[[x_1^{\pm 1}, \dots, x_n^{\pm 1}]]$ a module over the polynomial algebra $\mathbb{C}[x_1, \dots, x_n]$.
...
\end{equation}
A vertex algebra with a Hamiltonian operator is called \emph{graded}.
...
Let $M$ be a module over a graded vertex algebra $V$ with Hamiltonian operator $H$.
...
\begin{theorem}[{\cite[Theorem 2.15]{li_vertex_2004}}]
  \label{thr:8}
  Let $(\mathfrak{g}, \mathfrak{F}, T_0)$ be a regular formal distribution Lie algebra with $\mathfrak{F} = \{a^j(z)\}_{j \in J}$.
  Given a smooth $(\mathfrak{g}, \mathfrak{F}, T_0)$-module $M$, there is a unique module structure $Y^M: V(\mathfrak{g}, \mathfrak{F}, T_0) \to \mathcal{F}(M)$ such that for $j \in J$ and $u \in M$,
  \begin{equation}
    \label{eq:5}
    Y^M(a^j_{(-1)}\vac, z)u = a^j(z)u.
  \end{equation}
  Let $\lambda: \mathfrak{h} \to \mathbb{C}$ be a linear functional, where $\mathfrak{h}$ is a subalgebra of $\mathfrak{g}_+$ with $\mathfrak{h} \subseteq \ker(T_0)$.
  Given a smooth $(\mathfrak{g}, \mathfrak{F}, T_0)$-module $M$, where $h$ acts as $\lambda$, there is a unique module structure $Y^M: V^{\lambda}(\mathfrak{g}, \mathfrak{F}, T_0) \to \mathcal{F}(M)$ such that for $j \in J$ and $u \in M$, \eqref{eq:5} holds.
\end{theorem}
...
A \emph{Hamiltonian operator of $V$} is simultaneously a Hamiltonian operator of $V$ as a commutative associative algebra (see \Cref{sec:grad-poiss-algeb}) and a Hamiltonian operator of $V$ as a vertex Lie algebra.
...
Let $M$ be a module over a graded vertex Poisson algebra $V$ with Hamiltonian operator $H$.
A \emph{Hamiltonian operator of $M$} is simultaneously a Hamiltonian operator of $M$ as a module over $V$ as a commutative associative algebra with unit $1$ and a Hamiltonian operator of $M$ as a module over $V$ as a vertex Lie algebra.
...
If $R$ is a (graded) Poisson algebra, and $M$ is an $R$-module, then we can verify that $JR \otimes_{R} M$ is a (graded) $JR$-module by defining the Poisson structure as
\begin{equation*}
  a_{(n)}(b\otimes u) = (a_{(n)}b)\otimes u + \delta_{n, 0}b\otimes\{a, u\} \quad \text{for $a \in R$, $b \in JR$, $u \in M$ and $n \in \mathbb{N}$}
\end{equation*}
and $H^{JR \otimes_{R} M}$ as in \Cref{sec:graded-jet-algebras}.
We have a natural (graded) inclusion $\inc: M \hookrightarrow JR \otimes_R M$, which is a (graded) $R$-module homomorphism.
The (graded) $JR$-module $M$ together with the (graded) $R$-module monomorphism $\inc: M \hookrightarrow JR \otimes_R M$ satisfy a universal property similar to that of $JR$ (see \Cref{sec:graded-jet-algebras}), as we now show.
...
\begin{equation*}
  \Hom_{\{\text{(graded) $R$-modules}\}}(JR \otimes_R M, N) \cong \Hom_{\{\text{(graded) $JR$-modules}\}}(M, N).
\end{equation*}
...
\end{equation}
By \eqref{eq:6}, when $\dim(V_{\Delta}) < \infty$ for $\Delta \in \mathbb{C}$, it is natural to define the \emph{refined character of $V$ with respect to the Li filtration} as
\begin{equation*}
...
\end{equation}
By \eqref{eq:7}, when $\dim(M)_{\Delta} < \infty$ for $\Delta \in \mathbb{C}$, it is natural to define the refined character of $M$ with respect to the Li filtration as
\begin{equation*}
...
\begin{example}
  \label{exa:17}
  Let $c \in \mathbb{C}$.
  Then $\Vir^c$ is classically free because $\gr_F(\Vir^c) \cong \mathbb{C}[L_{-2}, L_{-3}, \dots]$ by \Cref{exa:14} and $JR_{\Vir^c} = J(C[L_{-2}]) = \mathbb{C}[L_{-2}, L_{-3}, \dots]$ by \Cref{exa:16} and  \Cref{sec:graded-jet-algebras}.

  If $c = c_{p, q}$ with $p > q \ge 2$ relatively prime integers, then $\Vir_{p, q} = \Vir_{c_{p, q}}$ is classically free if and only if $q = 2$ by \cite{van_ekeren_chiral_2021}.
\end{example}
...
\begin{example}
  ...
  \begin{equation*}
    R_{L(c, h)} \cong \frac{\bigoplus_{k \in \mathbb{N}}\mathbb{C}[L_{-2}]L_{-1}^k}{\sigma_0(J(c_{2, 2s + 1}, h_{1, i}))}.
  \end{equation*}
\end{example}
...
\begin{example}
  \label{exa:20}
  We pick a highest weight $(c, h)$.
  The Verma module $M(c, h)$ is always classically free as a $\Vir^c$-module because by \Cref{exa:15}, \Cref{exa:16} and \Cref{exa:19},
  \begin{align*}
...
We wish to determine when the irreducible modules $L(c_{p, q}, h_{m, n})$ over the simple Virasoro vertex algebras $\Vir_{p, q} = \Vir_{c_{p, q}}$ are classically free, see \Cref{thr:9} for details.
...
\begin{proof}
  ...
  \item $\lambda = [\underbrace{a + 1, \dots, a + 1}_{s - d}, \underbrace{a, \dots, a}_d]$ for some $a \in \mathbb{Z}_+$ and $d \in \{1, \dots, s\}$.
    ...
    \item[Subcase $a = 1$] In the sum \eqref{eq:10}, if $k > d$ or $k < d - 1$ or $n_i \neq 0$ for some $i = 1, \dots, s - 1$, then $R^{l, k}_{n_1, \dots, n_{s - 1}}(L(z), \dots, L(z))|c_{2, 2s + 1}, h_{1, i}\rangle$ contributes $0$.
      ...
      Since $\binom{s - 1}{d - 1} + \binom{s - 1}{d} = \binom{s}{d}$, we obtain \eqref{eq:9}.
    \item[Subcase $a > 1$] In the sum \eqref{eq:10}, if $k > 0$, then $R^{l, k}_{n_1, \dots, n_{s - 1}}(L(z), \dots, L(z))|c_{2, 2s + 1}, h_{1, i}\rangle$ contributes $0$.
      We have
      \begin{equation*}
  ...
  Thus, \eqref{eq:9} implies $\lp((L_{-2}^s\vac)_{(s - sa + d - 1)}|c_{2, 2s + 1}, h_{1, i}\rangle) = L_{\lambda}|c_{2, 2s + 1}, h_{1, i}\rangle$.
  From \eqref{eq:8} and \Cref{lmm:2}, we see that $\lp(u) = \lp((L_{-2}^s\vac)_{(s - sa + d - 1)}|c_{2, 2s + 1}, h_{1, i}\rangle) = L_{\lambda}|c_{2, 2s + 1}, h_{1, i}\rangle$.
  Finally, by \Cref{thr:9}, $u \in J(c_{2, 2s + 1}, h_{1, i})$, and it is clear that $u$ is homogeneous.
\end{proof}
...
\begin{proof}[Proof of \Cref{thr:3}]
  ...
  Given a partition $\lambda \in P^{s, i}$, we can write uniquely as $\lambda = \lambda^1 \cup \lambda^2$, where $\lambda^1 \in P^{s, 1}$ satisfies $\lambda^1 = \emptyset$ or $\lambda^1_{\len(\lambda^1)} \ge 3$ and $\lambda^2 = \emptyset$ or ($\lambda^2 \in P^{s, i}$ and $2 \ge \lambda^2_1$).
  ...
  \end{equation*}
  From \eqref{eq:11} and \eqref{eq:12}, we conclude that
  \begin{equation}
    \label{eq:13}
  ...
  \end{equation}
  By \Cref{lmm:8} and \Cref{thr:2}, the natural epimorphism $\phi_{L(c_{2, 2s + 1}, h_{1, i})}: JR_{\Vir_{2, 2s + 1}} \otimes_{R_{\Vir_{2, 2s + 1}}} R_{L(c_{2, 2s + 1}, h_{1, i})} \twoheadrightarrow \gr_F(L(c_{2, 2s + 1}, h_{1, i}))$ respects the grading while mapping the spanning set $\{t_{\lambda} \mid \lambda \in P^{s, i}\}$ onto a basis of $\gr_F(L(c_{2, 2s + 1}, h_{1, i}))$.
  Furthermore, we have $\dim((JR_{\Vir_{2, 2s + 1}} \otimes_{R_{\Vir_{2, 2s + 1}}} R_{L(c_{2, 2s + 1}, h_{1, i})})_{h_{1, i} + n})< \infty$ and $\dim(\gr_F(L(c_{2, 2s + 1}, h_{1, i}))_{h_{1, i} + n})< \infty$ for $n \in \mathbb{N}$.
  These observations, together with \Cref{prp:1} and \eqref{eq:13}, lead us to conclude that the natural epimorphism $\phi_{L(c_{2, 2s + 1}, h_{1, i})}$ is actually an isomorphism.
\end{proof}
...
A \emph{Hamiltonian operator of $A$} is a Hamiltonian operator $H$ of $A$ as a commutative associative algebra such that
\begin{equation*}
...
Let $A$ be an graded Poisson algebra with Hamiltonian $H$, and let $M$ be an $A$-module.
A Hamiltonian operator of $M$ is a  Hamiltonian operator $H^M$ of $M$ as a module over $A$ as a commutative associative algebra such that
\begin{equation*}
...
Assuming $R = \mathbb{C}[x^1, \dots, x^r]/(f_1, \dots, f_s)$ for some polynomials $f_i \in \mathbb{C}[x^1, \dots, x^r]$, the construction is as follows.
...
\end{equation*}
We set (identifying $x^j$ with $x^j_{(-1)}$ for $j = 1, \dots, r$ when considering $f_i$ in the following equation)
\begin{align*}
...
By our definitions, we can consider $\partial$ as a derivation of $JR$, and we have an algebra monomorphism $\inc: R \hookrightarrow JR, \inc(x^j + (f_1, \dots, f_s)) = x^j_{(-1)} + (f_1, \dots, f_s)_{\partial}$ for $j = 1, \dots, r$.
...
\begin{remark}
  \label{rmk:6}
  Identifying $x^j$ with $x^j_{(-1)}$, we see that the classes of the original variables $x^j$ generate $JR$ as a differential algebra, i.e.,
  \begin{equation*}
    JR = (x^1 + (f_1, \dots, f_s)_{\partial}, \dots, x^r + (f_1, \dots, f_s)_{\partial})_{\partial}.
  \end{equation*}
\end{remark}

If $f: R_1 \to R_2$ is a homomorphism of finitely generated algebras, then $Jf: JR_1 \to JR_2$ is defined by requiring that $f(\partial_1^n(\inc_1(x))) = \partial^n_2(\inc_2(f(x)))$ for $x \in R_1$ and $n \in \mathbb{N}$ (c.f.\ \Cref{rmk:6}).
We have constructed a functor
\begin{equation*}
  J: \{\text{finitely generated algebras}\} \to \{\text{differential algebras}\},
\end{equation*}
such that for a finitely generated algebra $R$ and a differential algebra $A$, we have a natural isomorphism
\begin{equation*}
...
\begin{remark}
  ...
  However, the theory of jet algebras is usually done this way.

  Here is an alternative and idiomatic way to obtain a functor
  \begin{equation*}
  ...
  For an algebra (not necessarily finitely generated) $R$, we set:
  \begin{align*}
    JR &= R \otimes t^{-1}\mathbb{C}[t^{-1}], \\
    \partial &= -\partial_t, \\
    \inc: R &\to JR, \\
    \inc(x) &= xt^{-1}.
  \end{align*}
  As before $\inc: R \to JR$ satisfies the same universal property, and we have
  \begin{equation*}
    JR = (\inc(R))_{\partial}.
  \end{equation*}
  This construction is probably better suited for the theory of vertex algebras, and we will use it in this article.
\end{remark}
...
Again, the $JR$-module $M$ together with the $R$-module monomorphism $\inc: M \hookrightarrow JR \otimes_R M, \inc(u) = 1\otimes u$ satisfy a universal property similar to that of $\inc: R \hookrightarrow JR$, and the functor $JR \otimes_R \bullet$ is left adjoint to the forgetful functor $\text{$JR$-Mod} \to \text{$R$-Mod}$.
